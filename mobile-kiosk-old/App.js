...
import { StyleSheet, Text, View, Button, Dimensions, ActivityIndicator, Alert, TouchableOpacity } from 'react-native';
import { Camera, CameraView } from 'expo-camera';
import axios from 'axios';

// The URL of your local or cloud HR system
...
export default function App() {
  ...
  const handleBarCodeScanned = async ({ type, data }) => {
    ...
    try {
      ...
      console.error(err);
      setScanStatus('error');
      setResultMessage('Network Error. Ensure the API URL is correct and the server is running.');
    } finally {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
      ...
      <View style={styles.scannerWrapper}>
        ...
        >
          {/* Overlay border to guide the user */}
          <View style={[styles.overlayBox, { borderColor: colors.box }]}>
            {(scanStatus === 'idle' && !scanned) && (
