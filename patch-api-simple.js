...
const targetStr = `            if (!isManager && !isOps && !isPres && !isSuper) {
...
const insertStr = `            if (!isManager && !isOps && !isPres && !isSuper) {
                return NextResponse.json({ error: 'Unauthorized to approve at this stage' }, { status: 403 });
            }
...
if (content.includes(targetStr)) {
    ...
    console.log("Success backend logic edit!");
} else {
    console.log("Failed. Target string mismatch.");
}
